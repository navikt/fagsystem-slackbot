...
const setupJob = (app) => {
  const onTick = async () => {
    ...
    if (new Date().getDay() === 5) {
      title = "Hvor skal du jobbe på mandag?";
    } else {
    ...
    try {
      ...
      if (result.ok) {
        ...
        console.error(
          `Error on postMessage to channel ${channel}: ${result.error}`,
        );
      }
    } catch (e) {
      console.error(e);
  ...
  const time = "00 37 21 * * 1-5"; // kl 13:55:00, man-fre, alle uker, alle måneder
